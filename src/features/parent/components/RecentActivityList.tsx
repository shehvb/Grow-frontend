      ...
      <div className="space-y-4">
        {activities.map((activity, index) => {
          ...
          return (
            <div 
              key={index} 
              className="flex items-center justify-between p-5 bg-white rounded-[24px] shadow-sm border border-slate-100 hover:shadow-md transition-shadow group"
            >
              <div className="flex items-center gap-5">
                <div className={`w-[52px] h-[52px] rounded-[18px] flex items-center justify-center ${
                  ...
                  {isCompleted ? <FiCheck className="w-6 h-6 stroke-[3px]" /> : <MdBook className="w-6 h-6" />}
                </div>
